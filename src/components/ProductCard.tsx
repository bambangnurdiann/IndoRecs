...
import { ShoppingBag, Share2, Heart, ThumbsUp, ThumbsDown, ChevronDown, ChevronUp, BadgeCheck } from 'lucide-react';
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {product.community_verified && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400"><BadgeCheck className="w-3 h-3" /> Verified</span>}
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <ul className="space-y-1">{product.pros.slice(0, 3).map((pro, i) => (<li key={i} className="text-sm text-gray-500 dark:text-gray-400">{pro}</li>))}</ul>
          </div>
          <div>
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1.5">Kekurangan</h4>
            <ul className="space-y-1">{product.cons.slice(0, 3).map((con, i) => (<li key={i} className="text-sm text-gray-500 dark:text-gray-400">{con}</li>))}</ul>
          </div>
        </div>
        ...
        <div>
          ...
          {showSpecs && (<div className="mt-2 flex flex-wrap gap-1.5">{product.key_specs.map((spec, i) => (<span key={i} className="px-2.5 py-1 bg-gray-50 dark:bg-gray-800 rounded-lg text-sm text-gray-500 dark:text-gray-400">{spec}</span>))}</div>)}
        </div>

        <div className="flex gap-2 pt-1">
          <a href={product.affiliate_url || product.shopee_url} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-white dark:bg-gray-900 text-[#EE4D2D] text-sm font-medium rounded-lg border border-[#EE4D2D]/30 hover:bg-[#EE4D2D] hover:text-white transition-colors"><ShoppingBag className="w-3.5 h-3.5" />Shopee</a>
        </div>
