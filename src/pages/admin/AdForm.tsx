  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const adData = {
        ...
        end_date: Timestamp.fromDate(new Date(formData.end_date)),
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input 
              ...
              className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
              placeholder="Contoh: Promo Laptop Gaming Akhir Tahun"
              required
            />
          </div>
          ...
          {/* Target URL */}
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">Link Tujuan (Target URL)</label>
            <input 
              type="url"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Placement */}
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Penempatan</label>
              <select 
            ...
            {/* Priority */}
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Prioritas (Opsional)</label>
              <input 
                type="number"
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 py-4 bg-green-600 text-white font-black uppercase tracking-widest rounded-xl hover:bg-green-700 shadow-lg shadow-green-100 transition-all active:scale-95 disabled:opacity-50"
          >
            {isLoading ? (
